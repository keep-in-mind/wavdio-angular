...
export class ExhibitService {
  ...
  public createExhibitLike(exhibit: Exhibit, like: Like): Observable<Exhibit> {
    return this.http.post<Exhibit>(`${this.url}/${exhibit._id}/like`, like).pipe(
      tap((updatedExhibit: Exhibit) => this.loggingService.logInfo(`Created exhibit like. (exhibit = ${JSON.stringify(updatedExhibit)})`)),
      catchError(this.handleError<Exhibit>(`createExhibitLike(exhibit = ${JSON.stringify(exhibit)}, like = ${JSON.stringify(like)})`))
    );
  }
  ...
  public deleteExhibitLike(exhibit: Exhibit, like: Like): Observable<Exhibit> {
    return this.http.delete<Exhibit>(`${this.url}/${exhibit._id}/like/${like._id}`).pipe(
      tap((updatedExhibit: Exhibit) => this.loggingService.logInfo(`Deleted exhibit like. (like = ${JSON.stringify(updatedExhibit)})`)),
      catchError(this.handleError<Exhibit>(`deleteExhibitLike(exhibit = ${JSON.stringify(exhibit)}, like = ${JSON.stringify(like)}`))
    );
  }
